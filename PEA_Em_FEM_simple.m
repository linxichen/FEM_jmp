...
parfor i = 1:length(peak_aidx)
	impulse_panel(i,:,:) = simforward_A(peak_inits(:,i),peak_aidx(i),-2,irf_periods,grids,param);
	control_panel(i,:,:) = simforward_A(peak_inits(:,i),peak_aidx(i),0,irf_periods,grids,param);
	GIRF_panel(i,:,:) = impulse_panel(i,:,:)-control_panel(i,:,:);
end
GIRF_peak_badshock = squeeze(mean(GIRF_panel));

parfor i = 1:length(trough_aidx)
	impulse_panel(i,:,:) = simforward_A(trough_inits(:,i),trough_aidx(i),-2,irf_periods,grids,param);
	control_panel(i,:,:) = simforward_A(trough_inits(:,i),trough_aidx(i),0,irf_periods,grids,param);
	GIRF_panel(i,:,:) = impulse_panel(i,:,:)-control_panel(i,:,:);
end
